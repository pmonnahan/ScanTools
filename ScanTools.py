...
class scantools:
    ...
    def recode(self, split_dir, pops="all", print1=False, mem=4000, numcores=1, partition="nbi-medium", time="2-00:00"):
    ...
    def repolarize(self, recode_dir, repolarization_key, pops="all", time='0-02:00', mem='8000', numcores=1, partition="nbi-medium"):
    ...
    def calcAFS(self, recode_dir, data_name, window_size="50000", num_reps="200", sampind="-99", pops="-99", time="0-00:30", mem="8000", use_repol=True, print1=False, allow_one_missing=True, partition="nbi-short"):
        ...
        for pop in pops:
            ...
            shfile3.write('#!/bin/bash\n' +
                          ...
                          '#SBATCH --mem=' + str(mem) + '\n' +
                          'source python-3.5.1\n' +
                          'python3 ' + self.code_dir + '/calcAFS.py -i ' + infile + ' -o ' + recode_dir + ' -p ' + prefix + ' -nrep ' + num_reps + ' -ws ' + window_size + ' -sampind ' + str(si) + '\n')
            shfile3.close()
            if print1 is False:
